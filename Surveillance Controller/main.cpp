...
void irq_handler() {
    queue.call(process_distance);//queue calls the function process_distance
}

int main() {
    event_thread.start(callback(&queue, &EventQueue::dispatch_forever));//creates a infinitly running thread that executes whats in the queue
    ...
    while (true) {
        ...
        printf("Running\n");
        fflush(stdout);//print statement only shows once flushed due to event queue buffer
    }
